#ifndef BWCOMPRESSION_H_INCLUDED
#define BWCOMPRESSION_H_INCLUDED
...
class GrayscaleCompression
{
public:
	static void readFromBmPtoListG (BMP &image, std::vector<uint8_t> &grayscale)
	{
		for (auto y = 0; y < image.TellHeight(); y++)
			for (auto x = 0; x < image.TellWidth(); x++)
			{
				const auto temp = image.GetPixel(x, y);
				const uint8_t bwValue = 0.299*temp.Red + 0.587*temp.Green + 0.114*temp.Blue;
				grayscale.push_back(bwValue >> 4 << 4);
			}

		reverse(grayscale.begin(), grayscale.end());
	}
	...
	static void lz77CompressionG(std::vector<uint8_t> &grayscale, BMP &image)
	{
		std::string outname;
		std::cout << "Output file name: ";
		std::cin >> outname;

		std::ofstream file(outname.c_str(), std::ifstream::binary);
...
#endif // BWCOMPRESSION_H_INCLUDED
